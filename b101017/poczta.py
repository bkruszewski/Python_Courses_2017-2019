...
class Poczta(object):
    ...
    def __init__(self, login, haslo, serwer = "smtp.gmail.com"):
        self.login = login
        self.haslo = haslo
        self.server = serwer

    def wyslij_wiaomowsc(self, adresaci, temat, tresc, pliki = None):

        assert isinstance(adresaci, list) # sprawdza czy adresat jest instancja klasy list
        ...
        for plik in pliki or []:
            with open(plik, "rb") as zalacznik:
                ...
                part["Content - Disposition"] = \
                    f'attachemnt; filename="{basename(plik)}"'
